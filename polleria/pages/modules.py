...
from polleria.components.widgets import empty_state, kpi_card, labeled_input, labeled_native_select, labeled_select, status_badge, table_wrap
from polleria.constants import EXPENSE_CATEGORIES, INVENTORY_TYPES, PAYMENT_METHODS, PERIODS, PRODUCT_CATEGORIES, ROLE_LABELS, UNITS
                ...
                table_wrap(
                    rx.table.root(
                        ...
                        rx.table.body(rx.foreach(ReportState.sales, lambda s: rx.table.row(
                            ...
                            rx.table.cell(s.total_fmt), rx.table.cell(s.metodo_pago), rx.table.cell(s.ganancia_fmt),
                        ))),
                ...
                table_wrap(
                    rx.table.root(
                        ...
                        rx.table.body(rx.foreach(ReportState.expenses, lambda e: rx.table.row(
                            rx.table.cell(e.fecha), rx.table.cell(e.categoria), rx.table.cell(e.descripcion), rx.table.cell(e.importe_fmt),
                        ))),
                        width="100%", variant="surface",
                    )
                ),
...
def cash_page() -> rx.Component:
    return app_shell(
        ...
        rx.grid(
            ...
            kpi_card("Mercado Pago", CashState.mercadopago_fmt, "smartphone", "cyan"),
            kpi_card("Total general", CashState.total_fmt, "wallet", "orange"),
        ...
        table_wrap(
            rx.table.root(
                rx.table.header(rx.table.row(
                    ...
                    rx.table.column_header_cell("Usuario"),
                    rx.table.column_header_cell("Total"),
                    rx.table.column_header_cell("Esperado"),
                    rx.table.column_header_cell("Contado"),
                    rx.table.column_header_cell("Diferencia"),
                )),
                rx.table.body(rx.foreach(CashState.history, lambda c: rx.table.row(
                    rx.table.cell(c.fecha), rx.table.cell(c.usuario), rx.table.cell(c.total_fmt),
                    rx.table.cell(c.efectivo_esperado_fmt), rx.table.cell(c.efectivo_contado_fmt), rx.table.cell(c.diferencia_fmt),
                ))),
                width="100%", variant="surface",
            )
        ),
